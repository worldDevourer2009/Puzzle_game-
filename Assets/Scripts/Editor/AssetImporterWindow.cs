        ...
        void DrawBulkSettingsControls()
        {
            switch (_selectedAssetType)
            {
                ...
                case AssetType.Sprite:
                    _targetTextureCompression =
                        (TextureImporterCompression)EditorGUILayout.EnumPopup("Compression", _targetTextureCompression);
                    _targetCrunchCompression = EditorGUILayout.Toggle("Use crunch", _targetCrunchCompression);
                    break;
                case AssetType.Mesh:
                    _targetMeshCompression =
                        (ModelImporterMeshCompression)EditorGUILayout.EnumPopup("Mesh compression",
                            _targetMeshCompression);
                    break;
            }
        }
        ...
        void DrawAssetInfo(AssetInfo info)
        {
            ...
            switch (info.Importer)
            {
                case TextureImporter ti:
                    ...
                    EditorGUILayout.LabelField($"Crunch: {(ti.crunchedCompression ? "On" : "Off")}",
            ...
            try
            {
                ...
                foreach (var guid in guids)
                {
                    ...
                    if (importer != null && asset != null)
                    {
                        ...
                        {
                            case AssetType.Texture:
                                typeMatch = importer is TextureImporter texImp &&
        ...
        void ApplyBulkSettings()
        {
            ...
            try
            {
                ...
                for (var i = 0; i < total; i++)
                {
                    ...
                    if (EditorUtility.DisplayCancelableProgressBar($"Applying settings to {_selectedAssetType}(s)",
                            $"Completed : {info.DisplayName} ({i + 1}/{total})", progress))
                    {
                        break;
                    }
                    ...
                    {
                        case TextureImporter ti:
                            if ((_selectedAssetType == AssetType.Texture &&
                                 ti.textureType != TextureImporterType.Sprite) ||
                                (_selectedAssetType == AssetType.Sprite &&
                                 ti.textureType == TextureImporterType.Sprite))
                            {
                                if (ti.textureCompression != _targetTextureCompression)
                                {
                                    ti.textureCompression = _targetTextureCompression;
                                    changed = true;
                                }

                                if (ti.crunchedCompression != _targetCrunchCompression)
                                {
                                    ti.crunchedCompression = _targetCrunchCompression;
                                    changed = true;
                                }
                            }
                        ...
                        case ModelImporter mi:
                            ...
                            {
                                if (mi.meshCompression != _targetMeshCompression)
                                {
                                    mi.meshCompression = _targetMeshCompression;
                                    changed = true;
                                }
                            }
